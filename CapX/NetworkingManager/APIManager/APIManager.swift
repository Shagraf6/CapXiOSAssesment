...
enum DataError: Error{
    ...
    case runtimeError(_ error:String)
            ...
            URLSession.shared.dataTask(with: request) { data, response, error in
                ...
                guard let response = response as? HTTPURLResponse, 200 ... 299 ~= response.statusCode else{
                    if let response = response as? HTTPURLResponse, 401 == response.statusCode{
                        ...
                        completionHandler(.failure(DataError.unAuthorized("\(decodedErrorObj?.message ?? "Validation Error:") \(self.getErrorString(errors: decodedErrorObj?.errors))" ?? "Un Authorized Error !")))
                        return
                    }
